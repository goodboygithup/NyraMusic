...
from Nyra import YouTube, app
from Nyra.core.call import Nyra
from Nyra.misc import SUDOERS, db
from Nyra.utils.database import (is_active_chat,
                                       is_music_playing, set_loop,
                                       is_nonadmin_chat, music_off,
                                       music_on)
from Nyra.utils.decorators.language import languageCB
from Nyra.utils.formatters import seconds_to_min
from Nyra.utils.inline.play import (stream_markup,
                                          telegram_markup)
from Nyra.utils.stream.autoclear import auto_clean
from Nyra.utils.thumbnails import gen_thumb
...
async def del_back_playlist(client, CallbackQuery, _):
    ...
    if command == "Pause":
        ...
        await Nyra.pause_stream(chat_id)
        ...
        await Nyra.resume_stream(chat_id)
        await CallbackQuery.message.reply_text(
            _["admin_4"].format(mention)
        )
    elif command == "Stop" or command == "End":
        await CallbackQuery.answer()
        await Nyra.stop_stream(chat_id)
        ...
        try:
            ...
            if not check:
                ...
                try:
                    return await Nyra.stop_stream(chat_id)
                except:
                    return
        except:
            try:
        ...
        if "live_" in queued:
            ...
            try:
                await Nyra.skip_stream(chat_id, link, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
            ...
            try:
                await Nyra.skip_stream(chat_id, queued, video=status)
            except Exception:
                return await CallbackQuery.message.reply_text(
                    _["call_9"]
                )
